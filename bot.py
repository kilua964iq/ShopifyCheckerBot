...
class ShopifyCheckerDirect:
    ...
    async def get_best_product(self, session: httpx.AsyncClient, site: str):
                        ...
                        for variant in product.get('variants', []):
                            if variant.get('available', False):
                                try:
                                    ...
                                    # نفضل المنتجات بسعر معقول (أقل من 100$)
                                    if price < best_price and price <= 100:
                                        best_price = price
                                        best_product = {
                                            'id': str(variant['id']),
        ...
        # إذا ما لقينا منتج بسعر معقول، نجيب أي منتج متاح
...
async def test_proxy_direct(proxy):
    ...
    try:
        ...
        if not parsed:
            return False        async with httpx.AsyncClient(proxy=parsed, timeout=15, verify=False) as session:
...
async def test_site_complete(site, proxy):
    """اختبار موقع كامل - منتجات + سلة شراء"""
    try:
        site_url = f'https://{site}'
        parsed = parse_proxy_ultimate(proxy)
        ...
        async with httpx.AsyncClient(**client_kwargs) as session:
            ...
            if add_resp.status_code in [200, 201, 302]:
                return True, f"Working - {product_title}"
            else:
...
print("🛒 Smart Site Filter: Products + Cart")
print("💰 Smart Product Selection: Under $100")
